...
export const CompanyProfileInput = z.object({
  ...
  customsDefaultHsCode: z.string().optional().nullable(),
  customsDefaultOriginCountry: z.string().optional().nullable(),
  // VAT rate in basis points (2000 = 20.00%). Used to derive HT/TVA/TTC on invoices.
  vatRate: z.coerce.number().int().min(0).max(10000).default(2000),
...
export async function upsertCompanyProfile(
  ...
  try {
    ...
    const data = {
      ...
      eori: parsed.data.eori || null,
      customsDefaultHsCode: parsed.data.customsDefaultHsCode || null,
      customsDefaultOriginCountry: parsed.data.customsDefaultOriginCountry || null,
